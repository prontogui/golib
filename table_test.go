...
func Test_TableAttachedFields(t *testing.T) {
	table := &Table{}
	table.PrepareForUpdates(key.NewPKey(), nil, getBogeyEventTimestampProvider())
	verifyAllFieldsAttached(t, table.PrimitiveBase, "Embodiment", "HeaderRow", "Rows", "Status", "Tag")
}

func Test_TableMake(t *testing.T) {
	table := TableWith{
		Embodiment: "paginated",
		HeaderRow:  []Primitive{TextWith{Content: "H1"}.Make(), TextWith{Content: "H2"}.Make()},
...
func Test_TableFieldSettings(t *testing.T) {
	...
	table.SetHeaderRowVA(TextWith{Content: "H1"}.Make(), TextWith{Content: "H2"}.Make())
	if len(table.HeaderRow()) != 2 {
		t.Errorf("Headings() returned %d items.  Expecting 2 items.", len(table.HeaderRow()))
	}

	h1, ok := table.HeaderRow()[0].(*Text)
	if !ok {
		t.Fatal("HeaderRow()[0] is not of type Text")
	}
...
func Test_TableGetChildPrimitive(t *testing.T) {
	...
	if locate(key.NewPKey(2, 0, 0)).Label() != "r0c0" {
		t.Fatal("LocateNextDescendant doesn't return a child for pkey 2, 0, 0.")
	}

	if locate(key.NewPKey(2, 0, 1)).Label() != "r0c1" {
		t.Fatal("LocateNextDescendant doesn't return a child for pkey 2, 0, 1.")
	}

	if locate(key.NewPKey(2, 1, 0)).Label() != "r1c0" {
		t.Fatal("LocateNextDescendant doesn't return a child for pkey 2, 1, 0.")
	}

	if locate(key.NewPKey(2, 1, 1)).Label() != "r1c1" {
		t.Fatal("LocateNextDescendant doesn't return a child for pkey 2, 1, 1.")
	}
}
